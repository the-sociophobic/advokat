...
export default Ember.Component.extend({
  ...
  actions: {
    ...
    filter() {
      var { type, property_type, category, rooms, area, lot, countryDistrict, cityDistrict } = this.getProperties('type', 'property_type', 'category', 'rooms', 'area', 'lot', 'countryDistrict', 'cityDistrict');
      var price = (this.get('type') === 'купить' ? this.get('buy') : this.get('sell'));
      
      let results = this.get('rentals').filter(function(rental) {        
